...
exports.updateLatestVersionChangelog = updateLatestVersionChangelog;
...
async function listTemplates(q, limit = 50, offset = 0) {
...
async function createTemplate(name, description, content_md, created_by) {
...
async function updateTemplate(id, fields, changelog) {
    const sets = [];
    const params = [];
    let idx = 1;
    if (fields.name != null) {
    ...
    if (fields.content_md != null) {
        sets.push(`content_md = $${idx++}`);
        params.push(fields.content_md);
    }
    if (!sets.length) {
...
async function listVersions(templateId, limit = 50, offset = 0) {
...
async function updateLatestVersionChangelog(templateId, changelog) {
