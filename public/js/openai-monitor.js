document.addEventListener('DOMContentLoaded', function() {
  ...
  const FREQUENT_REFRESH_INTERVAL = 3000; // 3 seconds
  const HOURLY_REFRESH_INTERVAL = 3600000; // 1 hour
  ...
  function startAutoRefresh() {
    ...
    // Use hourly interval if we've had successful responses before
    const interval = (successfulCheckCount > 2) ? HOURLY_REFRESH_INTERVAL : FREQUENT_REFRESH_INTERVAL;
    console.log(`OpenAI Monitor: Setting refresh interval to ${interval/1000} seconds`);
    
    autoRefreshInterval = setInterval(fetchEvents, interval);
  }
  ...
  function fetchEvents() {
    ...
    fetch(`/api/llm/events?provider=${provider}`)
      ...
      .then(data => {
        console.log('OpenAI Monitor: Received events:', data.events ? data.events.length : 0);
        events = data.events || [];
        updateApiStatus(true);
        
        // Update the events count
        if (eventsCountEl) {
          eventsCountEl.textContent = events.length.toString();
        }
        
        // Render the events
        renderEvents();
        
        // Increment successful check count and adjust refresh interval if needed
        successfulCheckCount++;
        if (successfulCheckCount === 3) {
          // After 3 successful responses, switch to hourly checks
          console.log('OpenAI Monitor: Switching to hourly checks after 3 successful responses');
          if (autoRefreshInterval) {
            stopAutoRefresh();
            startAutoRefresh(); // This will use the hourly interval now
          }
        }
      })
      .catch(error => {
